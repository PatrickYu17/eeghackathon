...
export function GenericCategoryModal({
  categoryName,
  initialItems,
  onConfirm,
  onClose,
  ...
  const [items, setItems] = useState<Item[]>(() => {
    ...
    return initialItems.map((i) => ({ ...i, selected: true }));
  });
  ...
  const addCustomItem = () => {
    ...
    setItems((prev) => [
      ...prev,
      { id: `item-${Date.now()}`, label: trimmed, qty: 0, selected: true },
    ]);
    setCustomInput("");
  };
    ...
    <div className="modal-backdrop" style={styles.overlay}>
      <div className="modal-content" style={styles.modal}>
        <div style={styles.header}>
          <div>
            <h2 style={styles.heading}>{categoryName} — what do you carry?</h2>
        ...
        <div style={styles.tableWrap}>
          {items.length === 0 ? (
            <div style={styles.emptyState}>
              <p style={styles.emptyText}>
                No items yet. Use the field below to add your first {categoryName.toLowerCase()}.
              </p>
            </div>
          ) : (
            <table style={styles.table}>
              <thead>
                <tr style={{ animation: "fadeInDown 0.3s ease" }}>
                  ...
                  <th style={{ ...styles.th, width: "120px", textAlign: "right" }}>
                    Current qty
                  </th>
                </tr>
              </thead>
                ...
                {items.map((item, index) => (
                  <tr
                    ...
                    style={{
                      ...
                      animation: `fadeInUp 0.3s cubic-bezier(0.25, 1, 0.5, 1) ${0.05 + index * 0.02}s both`,
                    }}
                      ...
                      >
                        {item.selected && (
                          <svg
                            ...
                            strokeWidth="3"
                            strokeLinecap="round"
                            style={{ animation: "checkPop 0.25s cubic-bezier(0.25, 1, 0.5, 1)" }}
                          >
          ...
          {/* Add custom item row */}
          <div style={{ ...styles.addRowStandalone, animation: "fadeInUp 0.3s ease 0.2s both" }}>
            <input
              style={styles.addInput}
              type="text"
              placeholder={`+ Add a specific ${categoryName.toLowerCase()}...`}
              value={customInput}
              onChange={(e) => setCustomInput(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && addCustomItem()}
            />
            <button className="btn-animate" style={styles.addBtn} onClick={addCustomItem}>
              Add
            </button>
          </div>
        </div>
        ...
        <div style={{ ...styles.btnRow, animation: "fadeInUp 0.35s cubic-bezier(0.25, 1, 0.5, 1) 0.25s both" }}>
          <button className="btn-animate" style={styles.backBtn} onClick={onClose}>
            Back
          </button>
          <button
